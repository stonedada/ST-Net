...
class SwinTransformerUNetParallel(nn.Module):
    def __init__(self, channels: Tuple[int], num_heads: int = 2, image_size: int = 128, is_residual: bool = False,
        ...
        self.encode = nn.ModuleList([EncoderLayer(channels[i], channels[i + 1], is_residual, bias).to('cuda:2') for i in
                                     range(len(channels) - 2)])  # transform 6 to 2
        self.bottle_neck = ConvBlock(channels[-2], channels[-1], is_residual, bias).to('cuda:2')  # transform 6 to 2
        self.pos_embed_mhsa = nn.Parameter(torch.randn(1, channels[-1], image_size // (2 ** (len(channels) - 2)),
                                                       image_size // (2 ** (len(channels) - 2)))).to(
            'cuda:2')  # transform 6 to 2
        self.mhsa = MultiHeadSelfAttention(channels[-1], num_heads, bias).to('cuda:1')
        self.pos_embed_x = nn.ParameterList(
            [nn.Parameter(torch.randn(1, channels[i + 1], image_size // (2 ** i), image_size // (2 ** i))).to('cuda:0')
             for i in reversed(range(1, len(channels) - 1))])
        self.pos_embed_skip_x = nn.ParameterList([nn.Parameter(
            torch.randn(1, channels[i], image_size // (2 ** (i - 1)), image_size // (2 ** (i - 1)))).to('cuda:0') for i
                                                  in reversed(range(1, len(channels) - 1))])
        self.mhca = nn.ModuleList(
            [MultiHeadCrossAttention(channels[i], num_heads, channels[i], channels[i + 1], bias).to(f'cuda:1') for i in
             reversed(range(1, len(channels) - 1))])
        self.decode = nn.ModuleList([DecoderLayer(channels[i + 1], channels[i], is_residual, bias).to('cuda:3') for i in
                                     reversed(range(1, len(channels) - 1))])  # transform 7 to 3
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        skip_x, x = self.patch_embed(x.to('cuda:2'))
        skip_x_list.append(skip_x)
        x = self.pos_drop(x)
        for layer in self.layers:
            skip_x, x = layer(x.to('cuda:2'))
            skip_x_list.append(skip_x)
        x = self.pool(skip_x)
        x = self.bottle_neck(x.to('cuda:2'))
        x = x.to('cuda:2') + self.pos_embed_mhsa
        ...
        for i, skip_x in enumerate(reversed(skip_x_list)):
            ...
            skip_x = self.mhca[i](skip_x.to('cuda:1'), x.to('cuda:1'))  # replace the self.cuda_cores
            x = self.decode[i](skip_x.to('cuda:3'), x.to('cuda:3'))  # transform 7 to 3
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = self.norm(x)
            skip_x = x
        skip_x = skip_x.permute(0, 2, 1).contiguous().view(B, self.embed_dim, H, W)
        return skip_x, x
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer).to('cuda:2')
