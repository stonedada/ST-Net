...
from Network.models.EncoderDecoder import ConvBlock, EncoderLayer, DecoderLayer, CNNDecoderLayer
...
class SwinTransformerUNetParallel(nn.Module):
    def __init__(self, channels: Tuple[int], num_heads: int = 2, image_size: int = 128, is_residual: bool = False,
        ...
        self.decode = nn.ModuleList(
            [CNNDecoderLayer(channels[i + 1], channels[i], is_residual, bias).to('cuda:3') for i in
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, skip_x in enumerate(reversed(skip_x_list)):
            ...
            x = self.decode[i](x.to('cuda:3'))  # transform 7 to 3
